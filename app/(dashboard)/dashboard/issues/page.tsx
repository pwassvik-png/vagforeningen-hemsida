    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        {issues.map((issue) => {
          ...
          return (
            <div key={issue.id} className="bg-white rounded-2xl p-5 sm:p-6 border border-outline-variant shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <h2 className="text-lg font-semibold mt-1.5 text-on-surface">{issue.category}</h2>
                  <p className="text-sm text-on-surface-variant mt-1">
                    {issue.user_name} • {new Date(issue.created_at).toLocaleDateString("sv-SE")}
                  </p>
                  {issue.location && (
                    <p className="text-sm text-on-surface-variant mt-1">📍 {issue.location}</p>
                  )}
                </div>
                {canChangeStatus && (
                  <select
                    ...
                    className="text-sm border border-outline-variant rounded-[10px] px-3 py-1.5 bg-surface text-on-surface shrink-0"
                  >
                    {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                )}
              </div>
              <p className="mt-3 text-on-surface-variant leading-relaxed">{issue.description}</p>
            </div>
          );
        })}
      </div>
